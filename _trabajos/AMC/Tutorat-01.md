...
| 主题    | 大致题目数量 | 内容           | 内容          | 内容      | 内容  |
| ----- | ------ | ------------ | ----------- | ------- | --- |
| 几何    | 7-9    | 三角形&长方形&勾股定理 | 相似&角度       | 立体      |     |
| 代数    | 6-7    | 数列           | 求和技巧（裂项&配对） | 方程      | 不等式 |
| 数论    | 5      | 整除           | 加减乘除        | 简化数字的问题 |     |
| 计数/概率 | 4-5    | 排列组合         |             |         |     |
| 比例/逻辑 | 1-2    | 比例           | 逻辑          |         |     | -->
...
- 基础题分类讲解视频：<https://www.omegalearn.org/amc8-fundamentals>
- 提高题分类讲解视频：<https://www.omegalearn.org/amc8-advanced>
- 往年真题：<https://artofproblemsolving.com/wiki/index.php/AMC_8_Problems_and_Solutions>
- 往年真题和模拟题：<https://www.isinj.com/mt-amc8/>
...
![](attachments/Pasted%20image%2020250906221125.png)
...
$\textbf{(A)}\ \text{equilateral triangle}$ 等边三角形 $\textbf{(B)}\ \text{non-square rhombus}$ 非正方形的菱形 $\textbf{(C)}\ \text{non-square rectangle}$ 非正方形的长方形 $\textbf{(D)}\ \text{isosceles trapezoid}$ 等腰梯形 $\textbf{(E)}\ \text{square}$ 正方形
...
如果只使用一分钱、五分、十分和二十五分的硬币，弗雷迪最少需要多少枚硬币才能支付任何少于一美元的金额？
...
Emily 在路上骑车。Emerson 往**同方向**（上课说错了，说成了反方向）骑。
一开始 Emily 看到 Emerson 在前方 $1/2$ 英里处。
过了一会儿，Emily 从后视镜看到 Emerson 在后方 $1/2$ 英里处。
已知 Emily 速度是 $12$ 英里/小时，Emerson 速度是 $8$ 英里/小时。
那么这前后经过了多少分钟？
...
Ryan 做了三套题，第一套题 $25$ 题，答对 $80\%$；第二套题 $40$ 题，答对 $90\%$；第三套题 $25$ 题，答对 $70\%$。答对的题数占总题数的百分之多少？
...
意大利辣香肠半径 $= \frac{12/2}{6} = 1$ 英寸。$24$ 的话面积就是 $24 \times \pi \times 1^2 = 24 \pi$。
pizza 的面积是 $\pi (12/2)^2 = 36 \pi$。
百分之几？占比是 $\frac{24 \pi}{36 \pi} = \dots$
...
> 平时我们在外面买东西/吃饭的时候付款付的是 TTC （含税价格）。
> 例如：如果 TTC 是 $12$ 块钱，那么其中包含的 TVA 是多少钱？（答案：$2$ 块钱）
...
三角形三条边边长是连续的三个整数。最短的边长是周长的 $30\%$。那么最长的边长为多少？
...
一个罐子有五种软糖，$30\%$ 蓝，$20\%$ 棕，$15\%$ 红，$10\%$ 黄，剩下的 $30$ 个是绿色的。如果一半的蓝色换成棕色，那么棕色的比例变成了多少？
...
一个正方形和圆形有相同的面积。那正方形的边长和圆的半径比例是多少？
...
长度比例 $\frac{AD}{AB} = \frac{3}{2}$，且 $AB = 30$ 英尺。求长方形的面积和两个半圆形面积总和的比例。

![|200](attachments/Pasted%20image%2020250906221517.png)
...
![|300](attachments/Pasted%20image%2020250906221547.png)
...
有个三位数 $abc$，百位数比个位数多 $2$。把三位数倒过来写成 $cba$，那么 $abc - cba$ 的个位数是多少？
